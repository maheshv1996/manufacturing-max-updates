...
export default function OtRegisterPage() {
  ...
  // Resolve current user for permission gating
  useEffect(() => {
    fetch("/api/auth/me")
      .then(async (r) => {
        if (!r.ok) return null;
  ...
  const allowed =
    me === null || can(me, "reports.print") || can(me, "people.view");
  const denied = me !== null && !allowed;
    ...
    <div className="min-h-screen bg-bg text-slate-100">
      {denied ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          <p className="text-lg font-black text-rose-500">Access denied</p>
          <p className="text-sm text-slate-400">
            You need the reports.print or people.view permission to view the OT
            register.
          </p>
          <Link
            href="/reports"
            className="text-sm text-blue-600 hover:text-blue-500 font-bold mt-2"
          >
            Back to Reports
          </Link>
        </div>
              ...
              <div className="space-y-8 print:space-y-4">
                ...
                <div className="flex items-start justify-between print:mb-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1 print:text-xs">
                      <strong>Operator:</strong> {selectedOp?.name || "â€”"}{" "}
                      &nbsp;|&nbsp;
                  ...
                  <div className="text-right text-xs text-slate-500 print:text-[10px]">
                    ...
                    <div>
                      Multiplier: <strong>Ã—{data.multiplier}</strong>
                    </div>
                    <div>
                      Labor Rate: <strong>â‚¹{data.laborRate}/hr</strong>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-4 print:gap-2">
                  {[
                    ...
                    {
                      label: "Est. OT Pay",
                      value: `â‚¹${data.estimatedOtPay.toLocaleString("en-IN")}`,
                ...
                {data.totalOt > 50 && (
                  <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-800 rounded-xl p-4 flex items-center gap-3 text-sm font-bold text-amber-300 print:rounded-none">
                    âš ï¸ This operator has exceeded the 50-hour statutory
                    ...
                    <tbody className="divide-y divide-slate-700">
                      {data.rows.length === 0 ? (
                        <tr>
                          ...
                          >
                            No attendance records with clock-out for this
                            period.
                          </td>
                        </tr>
                      ) : (
                        data.rows.map((row: any, idx: number) => (
                          <tr
                            key={idx}
                            className={`${idx % 2 === 0 ? "" : "bg-slate-800/60"} hover:bg-blue-50/50 hover:bg-slate-800/90/60 transition-colors`}
                          >
                            ...
                            <td className="py-2.5 px-3 text-slate-600 text-slate-300 print:text-xs">
                              {row.shiftName}
                            </td>
                      ...
                      <tfoot className="border-t-2 border-slate-300 font-black">
                        ...
                        <tr className="bg-emerald-50 dark:bg-emerald-950/30">
                          <td
                            ...
                            className="py-3 px-3 text-right uppercase text-xs tracking-wider text-slate-300"
                          >
                            Estimated OT Pay ({data.totalOt}h Ã— â‚¹
                            {data.laborRate} Ã— {data.multiplier})
                          </td>
                          <td
                            colSpan={2}
                            className="py-3 px-3 font-mono text-xl text-emerald-400"
                          >
                            â‚¹{data.estimatedOtPay.toLocaleString("en-IN")}
                          </td>
                        </tr>
                      </tfoot>
                    )}
                ...
                <div className="flex justify-between text-[10px] text-slate-400 pt-4 border-t border-slate-700 print:text-[8px]">
                  <span>Manufacturing MES â€” OT Register</span>
