 ...
 * Grounded in precision discrete manufacturing, aerospace alloys, and ISO 3685 tool life standards.
 */
...
export interface MaterialMachinability {
  ...
  name: string;
  category:
    "STEEL" | "STAINLESS" | "ALUMINUM" | "TITANIUM" | "SUPERALLOY" | "BRASS";
...
export function calculateCncTurning(
  ...
  const {
    ...
    feedPerRevMm,
    depthOfCutApMm,
    noseRadiusREpsilonMm = 0.8,
    materialId = "STEEL-EN8",
    passesCount = 1,
    approachAngleDeg = 95,
  } = input;

  const mat = MATERIAL_DATABASE[materialId] || MATERIAL_DATABASE["STEEL-EN8"];

  // 1. Spindle Speed N = (Vc * 1000) / (PI * D)
  const spindleSpeedRpm = Math.round(
    (cuttingSpeedVcMMin * 1000) / (Math.PI * workpieceDiameterMm),
  );
  ...
  const chipThicknessHm = feedPerRevMm * Math.sin(kappaRad);
  const kc = mat.kc1 * Math.pow(Math.max(0.01, chipThicknessHm), -mat.mc);
  const machineEfficiency = 0.82; // standard CNC lathe efficiency
  const cuttingPowerKw =
    Math.round(
      ((metalRemovalRateCm3Min * kc) / (60 * 1000 * machineEfficiency)) * 100,
    ) / 100;

  // 5. Cutting Time Tc = (L / (fn * N)) * passes in seconds
  const singlePassTimeMinutes =
    feedRateMmMin > 0 ? lengthOfCutMm / feedRateMmMin : 0;
  const totalCuttingTimeSec =
    Math.round(singlePassTimeMinutes * 60 * passesCount * 10) / 10;

  // 6. Surface Roughness Ra ≈ (fn² / (32 * r_epsilon)) * 1000 (microns)
  const theoreticalRa =
    (Math.pow(feedPerRevMm, 2) / (32 * Math.max(0.1, noseRadiusREpsilonMm))) *
    1000;
  const estimatedSurfaceRoughnessRaMicrons =
    Math.round(theoreticalRa * 1.5 * 100) / 100;
  ...
  if (estimatedSurfaceRoughnessRaMicrons > 3.2) {
    recommendations.push(
      `Estimated surface finish Ra is ${estimatedSurfaceRoughnessRaMicrons} µm. For precision finish (Ra < 1.6 µm), reduce feed to ~${Math.round(Math.sqrt((1.6 * 32 * noseRadiusREpsilonMm) / 1500) * 100) / 100} mm/rev.`,
    );
  }
  ...
  return {
    ...
    chipThicknessHm: Math.round(chipThicknessHm * 1000) / 1000,
    recommendations,
  };
}
...
export function estimateToolLifeMinutes(
  ...
  taylorExponentN: number = 0.25,
): number {
  if (actualVc <= 0 || standardVc <= 0) return standardLifeMinutes;
  const life =
    standardLifeMinutes * Math.pow(standardVc / actualVc, 1 / taylorExponentN);
