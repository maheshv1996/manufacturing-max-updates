...
> **v2 rebuild status (C7 COMPLETE, 2026-09-05):** leave/attendance/payroll state core proven end-to-end on `v2` — leave machine `PENDING → APPROVED | REJECTED | CANCELLED` (engine-gated, reason-mandatory reject), attendance derivation (present/late/hours/OT) feeding payroll, `PayrollRun DRAFT` + payslips with engine-computed `lopDays`/`lopDeduction` per the schema formula, idempotent re-run, full audit trail. Session rotation (`src/lib/sessionRotation.ts`) implements the epoch-staleness check + fresh-token reissue half of the role-grant flow (W10 line 1). Smoke: `npm run test:c7-6` (CI-wired), 14/14 green on `mfgmax_v2_test`.
...
> **v2 rebuild status (C8 COMPLETE, 2026-09-05):** full state core implemented + DB-proven — PM rules (calendar + run-hour via RUNNING telemetry) auto-create jobs with no-duplicate rescan; breakdown jobs enforce RCA (rootCause always, countermeasure beyond 60 min — P28); tool life = consume→NEEDS_REGRIND→regrind (max regrinds → mandatory SCRAP) and cycle tools warn at threshold / RETIRE at max; calibration G-4: expired instrument auto-quarantined and refuses issue/measure until recalibrated; spare issue never goes negative with reorder flag at point; kit issues all-or-nothing (KIT_SHORTFALL lists what's missing); permit-to-work needs EHS + maintenance + production signatures (each reason-mandatory, per-leg route authz) before work is authorized. Smoke: `npm run test:c8-8` (CI-wired), 15/15 on `mfgmax_v2_test`. **C8-9 completion (same day):** the remaining W11 steps are closed — tool-life decrement runs inside v2 LOG_GOOD (warn → RETIRE at max cycles; unit dies → NEEDS_REGRIND with a single CONSUME alert on the crossing), G-4 blocks measurement at inspection time via `/api/v2/quality/inspections`, and machine DOWN/FAULT events auto-open BREAKDOWN jobs through `/api/v2/maintenance/breakdowns/scan` with duplicate suppression while open + re-open cooldown. Smoke now 20/20 on `mfgmax_v2_test`.
