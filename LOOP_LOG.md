...
| 15 | C11 AI Copilot Framework: 5 pure engines TDD (`seatContext.ts` + scope trimmer, `taskRouter.ts` + hardware tiering & multilingual prompts, `toolRegistry.ts`, `approvalBroker.ts` + G-1..G-6 guardrails, `fusion.ts` deterministic numbers), 1 adapter (`copilotTx.ts`), 4 API routes under `/api/v2/copilot/*`, real-DB smoke 11/11 | npm test 637/637 across 28 suites, tsc 0, cast scan clean, test:c11-11 11/11 on mfgmax_v2_test, verify-counts PASS (378 routes) | SUCCESS |
