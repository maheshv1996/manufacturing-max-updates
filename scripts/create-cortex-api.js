...
const apiDir = path.join('src', 'app', 'api', 'ai', 'cortex');
...
export const SAMPLE_CONFLICTS: ConflictScenario[] = [
  {
    ...
    description:
      "Sales committed delivery of 500x Titanium Turbine Blades in 5 days (\\,000 order). Line 1 5-Axis Mazak is already at 104% load, and on-hand Titanium 6Al-4V stock is 120 kg short.",
    options: [
      {
        ...
        impactOee: "+4.2%",
        impactCost: "+\\,850 (Expedited Freight)",
        impactDelivery: "100% On-Time (\\ Secured)",
      ...
      },
      {
        ...
        impactOee: "0.0%",
        impactCost: "\\ Extra",
        impactDelivery: "Lose \\ Rush Order",
        reasoning:
      ...
      },
      {
        ...
        impactOee: "-8.5%",
        impactCost: "+\\,400 (Downtime & Changeover)",
  ...
  },
  {
    ...
    description:
      "CNC-02 spindle bearing vibration RMS spiked to 4.82 mm/s (38.5 hrs RUL). A high-torque \\,000 Inconel Impeller job is scheduled to run on it today for 16 hours.",
    options: [
      {
        ...
        impactCost: "\\ (Planned PM)",
        impactDelivery: "On-Time (+1.5h shift catchup)",
        agentRecommendation: true,
        reasoning:
          "Prevents catastrophic \\,000 spindle replacement and 48-hour unplanned line stoppage while keeping the impeller batch on schedule.",
      },
      {
        ...
        impactOee: "-22.4%",
        impactCost: "\\,500 (Spindle Rebuild)",
        impactDelivery: "Line Down for 4 Days",
        reasoning:
          "High probability of catastrophic tool breakage, scrapped \\ Inconel casting, and emergency spindle replacement.",
      },
    ],
  },
];
    ...
    if (action === "simulate_what_if") {
      ...
      return NextResponse.json({
        ...
        simulation: {
          ...
          strategicInsights: [
            addMachines > 0 ? \Adding \ CNC machining centers unlocks +\\$\/mo in untapped aerospace contracts.\ : null,
            addShifts > 0 ? \Activating Shift \ spreads fixed overhead across 42% more machine hours.\ : null,
            nightTariffShiftHours > 0 ? \Shifting \h/day to night tariff saves \\$\/mo in utility costs.\ : null,
          ].filter(Boolean),
        },
      });
    }
...
console.log('Successfully written src/app/api/ai/cortex/route.ts');
